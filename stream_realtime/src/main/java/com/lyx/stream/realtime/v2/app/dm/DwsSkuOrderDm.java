...
//        env.getCheckpointConfig().setCheckpointStorage("hdfs://cdh01:8020/flink/checkpoints/DwsSkuOrder2");
        DataStreamSource<String> kafkaRead = SourceSinkUtils.kafkaRead(env, "dwd_trade_order_detail_yuxin_li");
//        kafkaRead.print();
...
//        kafkaRead.print();
        ...
        SingleOutputStreamOperator<JSONObject> filterDs = distinctDs.keyBy(o->o.getString("user_id")).filter(new FilterBloomDeduplicatorFunc(1000000, 0.01));
//        filterDs.print();
                ...
                AsyncDataStream.unorderedWait(filterDs,
                        ...
                        new RichAsyncFunction<JSONObject, JSONObject>() {
                            ...
                            public void asyncInvoke(JSONObject data, ResultFuture<JSONObject> resultFuture) throws Exception {
                                ...
                                JSONObject dimAsync = HbaseUtil.readDimAsync(hbaseCon, "ns_yuxin_li", "dim_sku_info", skuId);
                                if (dimAsync != null) {
...
//连接c1
        ...
        tm_connect.print();
        tm_connect.map(o->o.toJSONString()).sinkTo(SourceSinkUtils.sinkToKafka("yuxin_dws_sku_order_detail_v1"));
